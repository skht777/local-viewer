...
// --- thumbnail warm スキップ条件のテスト ---

/// `ThumbnailWarmer::new(0)` で起動した state を返す
/// concurrency=0 の Semaphore は permit 取得不能のため、
/// spawn したタスクが acquire().await で永遠にブロックされ、
/// pending エントリが消えないため `pending_count` を観測できる
fn test_state_with_blocked_warmer(
    root: &std::path::Path,
    mount_names: HashMap<std::path::PathBuf, String>,
) -> Arc<AppState> {
    let settings = Settings::from_map(&HashMap::from([(
    ...
    let video_converter = Arc::new(VideoConverter::new(Arc::clone(&temp_file_cache), &settings));
    // concurrency=0 で warm 完了を阻止し、pending を残す
    let thumbnail_warmer = Arc::new(ThumbnailWarmer::new(0));
    let index_db = tempfile::NamedTempFile::new().unwrap();
    let indexer = Arc::new(crate::services::indexer::Indexer::new(
        index_db.path().to_str().unwrap(),
    ));
...
#[tokio::test]
async fn limit付きcursor無しのbrowseはthumbnail_warmerをスケジュールする() {
    let (_dir, root) = create_test_dir();
    let state = test_state_with_blocked_warmer(&root, HashMap::new());
    let node_id = register_node_id(&state, &root);

    assert_eq!(state.thumbnail_warmer.pending_count(), 0);

    // limit=2, cursor 無し → 「ユーザーが直接ディレクトリを開いた最初のページ」
    ...
    // warm() がスケジュールされており pending が増えている
    assert!(
        state.thumbnail_warmer.pending_count() > 0,
        "limit付きcursor無しのbrowseでは warm() が呼ばれ pending_count > 0 になるべき"
    );
}

#[tokio::test]
async fn cursor付きの2ページ目browseはthumbnail_warmerをスケジュールしない() {
    // 4ファイル: cursor 付き 2 ページ目を取得できるディレクトリ
    let dir = TempDir::new().unwrap();
    let root = fs::canonicalize(dir.path()).unwrap();
    fs::write(root.join("a.jpg"), "1").unwrap();
    fs::write(root.join("b.jpg"), "2").unwrap();
    fs::write(root.join("c.jpg"), "3").unwrap();
    fs::write(root.join("d.jpg"), "4").unwrap();
    ...
    .await;
    let cursor = json1["next_cursor"].as_str().unwrap().to_string();
    let count_after_first = state.thumbnail_warmer.pending_count();
    assert!(count_after_first > 0, "1 ページ目で warm されているはず");

    // 2 ページ目: cursor 付きなので warm されない
    let (status2, _json2) = get_json(
        app(Arc::clone(&state)),
        &format!("/api/browse/{node_id}?limit=2&sort=name-asc&cursor={cursor}"),
    )
    .await;
    ...
    // pending_count は 1 ページ目から増加しないこと
    assert_eq!(
        state.thumbnail_warmer.pending_count(),
        count_after_first,
        "cursor 付き 2 ページ目では warm() を呼ばないため pending_count が増加してはならない"
    );
}

#[tokio::test]
async fn limit無しのbrowseはthumbnail_warmerをスケジュールしない() {
    // 内部探索用 (browseNodeOptions: limit パラメータ無し) は UI 表示外なので warm 不要
    let (_dir, root) = create_test_dir();
    let state = test_state_with_blocked_warmer(&root, HashMap::new());
    let node_id = register_node_id(&state, &root);
    ...
    let (status, _json) = get_json(
        app(Arc::clone(&state)),
        &format!("/api/browse/{node_id}?sort=name-asc"),
    )
    .await;
    assert_eq!(status, StatusCode::OK);

    // warm() は呼ばれないため pending_count は 0 のまま
    assert_eq!(
        state.thumbnail_warmer.pending_count(),
        0,
