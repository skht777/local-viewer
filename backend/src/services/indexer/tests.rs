...
fn scan_directoryでディレクトリを走査してインデックスに登録する() {
    ...
    let (count, report) = env
        .indexer
        .scan_directory(&env.root, &ps, "1111111111111111", 2, None)
        .unwrap();
...
fn incremental_scanで変更ファイルのみ更新する() {
    ...
    let (added, updated, deleted) = env
        .indexer
        .incremental_scan(&env.root, &ps, "1111111111111111", 2, None)
        .unwrap();
...
fn incremental_scanでネストされたディレクトリ内の新規ファイルを検出する() {
    ...
    indexer
        .scan_directory(&root, &ps, "abcdef0123456789", 2, None)
        .unwrap();
    ...
    let (added, _updated, deleted) = indexer
        .incremental_scan(&root, &ps, "abcdef0123456789", 2, None)
        .unwrap();
...
fn rebuildでインデックスを再構築する() {
    ...
    let count = env
        .indexer
        .rebuild(&env.root, &ps, "1111111111111111")
        .unwrap();
...
impl MultiMountEnv {
    ...
    fn new_with_initial_scan() -> Self {
        ...
        indexer
            .scan_directory(&root_a, &ps_a, "aaaaaaaaaaaaaaaa", 2, None)
            .unwrap();
        indexer
            .scan_directory(&root_b, &ps_b, "bbbbbbbbbbbbbbbb", 2, None)
            .unwrap();
...
fn incremental_scanは他マウントのエントリを削除しない() {
    ...
    env.indexer
        .incremental_scan(&env.root_a, &env.ps_a(), "aaaaaaaaaaaaaaaa", 2, None)
        .unwrap();
    env.indexer
        .incremental_scan(&env.root_b, &env.ps_b(), "bbbbbbbbbbbbbbbb", 2, None)
        .unwrap();
...
fn rebuildは他マウントのエントリを削除しない() {
    ...
    env.indexer
        .rebuild(&env.root_a, &env.ps_a(), "aaaaaaaaaaaaaaaa")
        .unwrap();
    env.indexer
        .rebuild(&env.root_b, &env.ps_b(), "bbbbbbbbbbbbbbbb")
        .unwrap();
...
fn mount_scope_rangeのsqlはscan_entriesにならない() {
    ...
    let (lo, hi) = super::helpers::mount_scope_range("aaaaaaaaaaaaaaaa").unwrap();
...
fn search_combined_scope_range_ftsなしケースでscan_entriesにならない() {
    ...
    let (indexer, _tmp) = setup_indexer();
    let conn = indexer.connect().unwrap();
    let (lo, hi) = super::helpers::prefix_scope_range("mount1");
...
fn prefix_scope_rangeはネスト_prefixで正しい境界を返す() {
    let (lo, hi) = super::helpers::prefix_scope_range("mount1/photos");
...
fn prefix_scope_rangeはliteral_percent文字を含むprefixで安全() {
    let (lo, hi) = super::helpers::prefix_scope_range("mount/dir_100%");
...
fn mount_scope_rangeはinvariant違反でrejectする() {
    use super::helpers::mount_scope_range;
    // 長さ違反
