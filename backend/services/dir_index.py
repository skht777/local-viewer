...
class DirIndex:
    ...
    def begin_bulk(self) -> None:
        ...
        """
        self._tls.bulk_conn = self._connect_for_bulk()
        self._tls.bulk_entries = []
        self._tls.bulk_meta = []

    def end_bulk(self) -> None:
        """バルク挿入モードを終了する (残りフラッシュ + 接続クローズ)."""
        conn = getattr(self._tls, "bulk_conn", None)
        if conn is None:
            return
        if getattr(self._tls, "bulk_entries", None) or getattr(
            self._tls, "bulk_meta", None
        ):
            self._flush_bulk()
        conn.close()
        self._tls.bulk_conn = None
    ...
    def _flush_bulk(self) -> None:
        ...
        if conn is None:
            return
        entries = getattr(self._tls, "bulk_entries", None)
        meta = getattr(self._tls, "bulk_meta", None)
        if entries:
            conn.executemany(
                """
    ...
    def query_sibling(
        ...
        direction: str,
        sort: str,
        set_kinds: tuple[str, ...] = ("directory", "archive", "pdf"),
    ) -> dict[str, object] | None:
        """次/前の兄弟セット候補を返す (sibling エンドポイント用).

        browse クエリと同じソート順で比較する:
        - name-asc/desc: (kind != 'directory'), sort_key の複合ソート
        - date-asc/desc: (mtime_ns, sort_key ASC) Windows Explorer 準拠
        """
        conn = self._connect()
        try:
            current_sort_key = encode_sort_key(current_name)
            kinds_sql = "(" + ", ".join(f"'{k}'" for k in set_kinds) + ")"
            is_next = direction == "next"

            if sort in ("name-asc", "name-desc"):
                row = self._sibling_name(
                    conn,
                    parent_path,
                    current_sort_key,
                    0 if current_is_dir else 1,
                    is_next,
                    sort == "name-asc",
                    kinds_sql,
                )
            else:
                row = self._sibling_date(
                    conn,
                    parent_path,
                    current_name,
                    is_next,
    ...
    def _sibling_name(
        ...
        result: sqlite3.Row | None = conn.execute(
            sql, (parent_path, current_kind_flag, current_kind_flag, current_sort_key)
        ).fetchone()
        return result
    ...
    def _sibling_date(
        ...
        name で逆引き (UNIQUE(parent_path, name) が保証)。
        """
        ...
        """  # noqa: S608
        result: sqlite3.Row | None = conn.execute(
            sql, (parent_path, current_mtime, current_mtime, current_sort_key)
        ).fetchone()
        return result
    ...
    def ingest_walk_entry(
        ...
        bulk_conn = getattr(self._tls, "bulk_conn", None)
        if bulk_conn is not None:
            bulk_entries = self._tls.bulk_entries
            for e_name, e_kind, e_size, e_mtime in entries:
                sk = encode_sort_key(e_name)
                bulk_entries.append((parent_path, e_name, e_kind, sk, e_size, e_mtime))
            self._tls.bulk_meta.append((parent_path, dir_mtime_ns))
            if len(bulk_entries) >= self._BATCH_SIZE:
                self._flush_bulk()
        else:
            if entries:
