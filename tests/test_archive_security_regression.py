...
async def test_ドットドットエントリを含むZIPのbrowseでエントリが除外される(
    ...
    """../を含むエントリはlist_entriesの検証で除外 or エラーになる."""
    ...
    # traversal エントリはセキュリティエラーで除外されるか、
    # 422 で全体が拒否される
...
async def test_zip_bombのbrowseでセキュリティエラー(
    client: AsyncClient,
    test_node_registry: NodeRegistry,
    test_root: Path,
) -> None:
    """圧縮率が異常に高いエントリを含む ZIP."""
    archive = test_root / "bomb.zip"
    with zipfile.ZipFile(archive, "w", zipfile.ZIP_DEFLATED) as zf:
        # 繰り返しパターンで高い圧縮率を実現
        huge_data = b"\x00" * (64 * 1024 * 1024)  # 64MB > 32MB 上限
