...
async def test_rebuildが全マウントのmount_id_mapでindexer_rebuildを呼ぶ(
    search_client: AsyncClient,
    test_indexer: Indexer,
    test_node_registry: NodeRegistry,
    test_root: Path,
) -> None:
    """rebuild エンドポイントが NodeRegistry.mount_id_map を使って
    全マウントを Indexer.rebuild に渡すことを検証する."""
    import asyncio
    from unittest.mock import AsyncMock, patch

    # 2つのマウントポイントを設定
    mount_b = test_root / "mount_b"
    mount_b.mkdir(exist_ok=True)
    test_node_registry.set_mount_id_map(
        {"default": test_root, "extra": mount_b}
    )

    # indexer.rebuild を spy して呼び出し引数をキャプチャ
    captured_args: list[dict[str, object]] = []
    original_rebuild = test_indexer.rebuild

    def spy_rebuild(mounts, path_security):
        captured_args.append({"mounts": mounts})
        return original_rebuild(mounts, path_security)

    with patch.object(test_indexer, "rebuild", side_effect=spy_rebuild):
        response = await search_client.post("/api/index/rebuild")
        assert response.status_code == 202
    ...
    passed_mounts = captured_args[0]["mounts"]
    assert "default" in passed_mounts
    assert "extra" in passed_mounts
    assert passed_mounts["default"] == test_root
    assert passed_mounts["extra"] == mount_b


async def test_空白のみのクエリで422を返す(search_client: AsyncClient) -> None:
    """空白のみ (strip 後に空) のクエリは 422 で拒否する."""
    response = await search_client.get("/api/search", params={"q": "   "})
    assert response.status_code == 422


async def test_前後空白がトリミングされて検索される(
    search_client: AsyncClient,
    test_indexer: Indexer,
    test_root: Path,
) -> None:
    """前後の空白がトリミングされた上で検索され、レスポンスの query もトリミング後の値."""
    img = test_root / "photo.png"
    rel = str(img.relative_to(test_root))
    test_indexer.add_entry(
        IndexEntry(rel, "photo.png", "image", 100, img.stat().st_mtime_ns)
