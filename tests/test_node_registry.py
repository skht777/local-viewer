...
def test_ディレクトリエントリのpreview_node_idsに動画node_idが含まれる(
    registry: NodeRegistry, root_dir: Path
) -> None:
    # 動画のみを含むディレクトリ
    video_only = root_dir / "video_only"
    video_only.mkdir()
    (video_only / "clip.mp4").write_bytes(b"\x00" * 1024)

    entries = registry.list_directory(root_dir)
    d = next(e for e in entries if e.name == "video_only")
    assert d.preview_node_ids is not None
    assert len(d.preview_node_ids) >= 1
