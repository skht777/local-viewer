...
# マウントポイント管理 TUI
#
# ホスト側で実行し、config/mounts.json と docker-compose.yml を更新する。
# 依存: jq, uuidgen
#
# 使い方:
#   ./manage_mounts.sh
...
START_MARKER="# --- managed mounts (do not edit) ---"
END_MARKER="# --- end managed mounts ---"
...
# docker-compose.yml のマーカー区間を mounts.json から再生成する
sync_compose() {
    if [[ ! -f "$COMPOSE_FILE" ]]; then
        echo "警告: docker-compose.yml が見つかりません"
        return
    fi

    if ! grep -q "$START_MARKER" "$COMPOSE_FILE"; then
        echo "警告: docker-compose.yml にマーカーコメントがありません"
        return
    fi

    # mounts.json から volume 行を生成
    local mount_lines=""
    ...
    awk -v start="$START_MARKER" -v end_marker="$END_MARKER" -v mounts="$mount_lines" '
        index($0, start) { print; if (mounts != "") print mounts; skip=1; next }
