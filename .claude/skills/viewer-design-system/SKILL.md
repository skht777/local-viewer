...
| 階層 | クラス | 用途 | 使用例 |
|------|--------|------|--------|
| Primary | `text-white` / `text-gray-50` | 見出し、アクティブ状態 | ファイル名、選択中タブ |
| Secondary | `text-gray-300` | インタラクティブなセカンダリ要素 | ツールバーボタン、ヘッダーリンク |
| Tertiary | `text-gray-400` | 情報的なセカンダリ要素 | ファイルサイズ、非アクティブタブ、ローディング |
| Subdued | `text-gray-500` | 補助ラベル、パス、空状態 | ツリーラベル、検索パス、empty state |
| Muted | `text-gray-600` | プレースホルダ専用 | input placeholder |

### アクセントカラー

- **確定**: `blue-500` / `blue-600`（フォーカスリング、アクティブ状態、選択ハイライト）
- ダークテーマとのコントラスト比が十分で、コンテンツビューワーの「静かな上質感」に合致
- `--color-accent` トークンは導入しない（10箇所程度で grep 管理可能な規模）
